...
for word in dftopListWords:
    for matchWord,matchval in process.extract(word, words,limit=10):
        if word != matchWord:
            levnDist = jellyfish.levenshtein_distance(word, matchWord)
            seqMatch = round(SequenceMatcher(None, word, matchWord).ratio() * 100)
            # fuzz.token_sort_ratio
            matchAnalysis.append([word,matchWord, matchval,levnDist,seqMatch])
    ...
    dfMatch = pd.DataFrame.from_records(matchAnalysis,
                                         columns=["word", "matchedWord", "matched", "levnDist", "seqMatch"])
